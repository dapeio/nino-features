...
| Path | Ownership |
| --- | --- |
| `features/<Name>/` | One feature, exactly what lands in a project's `features/`. Thirteen today: `Consent`, `Design`, `Forms`, `Gallery`, `Lightbox`, `Mailer`, `Newsletter`, `ProtectedArea`, `Search`, `Seo`, `Stats`, `Templates`, `Typewriter`. The directory name is the class name `\Nino\Modules\<Name>` and MUST match `/^[A-Z][A-Za-z0-9]*$/` |
| `design-library/` | Parked material, not a feature and never published: the ten whole-page themes Nino's setup wizard offered up to 1.1, the 1.1 Design panel's archived manual (`docs/appearance.md`/`.de.md`, kept for the token contract), the concept the Design feature was built from (`docs/design-feature.md`, German), and `preview.php` - a dev-only harness that boots a real Nino and compiles a specimen page through `features/Design/`'s own `Setup` and `Compiler`, so part sets can be designed without a workbench. It runs under `php -S` and, as a front controller that answers every path itself, behind nginx/php-fpm; `PREVIEW_KEY` is what opens it past the loopback and there is no other gate, so never weaken it. The headers and footers moved into `features/Design/library/` when that feature was written. `bin/build.php` and `bin/check.sh` only ever read `features/`, so nothing here is packed, listed or tested - CI lints `preview.php` with `php -l` and nothing else runs it |
...
| File | Requirement |
| --- | --- |
| `feature.php` | `key` (a slug), `name` (no two features may carry one, in any locale, case ignored - `bin/build.php` refuses a pair that does), `description`, `manual` (how the feature is used, which the panel puts at the top of its screen - two paragraphs at most, `` `backticks` `` for code; write it as a nowdoc) (a string or a `locale => string` map, `en_US` and `de_DE` at least), `category` (one of `content`, `ui`, `communication`, `marketing`, `security`, `system` - `bin/build.php` refuses anything else, and refuses a feature without one), `version` (`major.minor.patch`), `nino` (a version constraint, `^1.0` today), `requires`, `settings`, `data` - every key spelled out, even when empty, so a reader sees what the feature does not do |
