...
| Path | Ownership |
| --- | --- |
| `features/<Name>/` | One feature, exactly what lands in a project's `features/`. Twelve today: `Consent`, `Forms`, `Gallery`, `Lightbox`, `Mailer`, `Newsletter`, `ProtectedArea`, `Search`, `Seo`, `Stats`, `Templates`, `Typewriter`. The directory name is the class name `\Nino\Modules\<Name>` and MUST match `/^[A-Z][A-Za-z0-9]*$/` |
| `design-library/` | Parked material, not a feature and never published: the ten themes, six headers and seven footers Nino's setup wizard offered up to 1.1, waiting for the **Design** feature. `bin/build.php` and `bin/check.sh` only ever read `features/`, so nothing here is packed, listed or tested. Its own `README.md`/`README.de.md` say what each unit is and what a header preset has to keep; `docs/appearance.md`/`.de.md` is the 1.1 Design panel's archived manual, kept for the token contract |
