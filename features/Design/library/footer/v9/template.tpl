		<footer class="nino-frame-footer nino-frame-footer--sitemap">
			<div class="nino-grid-row">
				<div class="nino-grid-100 nino-grid-l-33 nino-frame-footer-brand-col">
					<a href="[[/nino/dir]]/" class="nino-frame-footer-brand">
						<img src="[[/nino/public]]/images/logo.png" class="nino-footer-logo" alt="[[/company/name]]">
					</a>
					<p class="nino-frame-footer-description">[[/company/description]]</p>
					<address class="nino-frame-footer-contact">
						<p>[[/company/adress]]<br>[[/company/country]]</p>
						<p>[[/company/phone]]<br>[[/company/email]]</p>
					</address>
				</div>
				<nav class="nino-grid-100 nino-grid-l-66 nino-frame-footer-nav" aria-label="[[/website/footer/title/navigation]]">
					[template /templates/html-footer-nav]
				</nav>
			</div>
			<div class="nino-grid-row">
				<div class="nino-grid-100 nino-frame-footer-legal">
					<p>&copy; [[/date/year]] [[/company/name]]</p>
					<div class="nino-frame-footer-utility">
						[template /templates/html-footer-legal]
						[template /templates/html-footer-localepicker]
					</div>
				</div>
			</div>
		</footer>
